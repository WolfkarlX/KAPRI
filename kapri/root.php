...
?>
<!--Hoja de impresion de las publicaciones como administrador root-->
<!DOCTYPE html>
<html lang="es">
    <head>  
    	<meta charset="UTF-8">
    ...
    <body>
        ...
        <?php if(isset($_REQUEST["info"])){ ?>
            <?php if($_REQUEST["info"] == "ZDYXY47864PDLDJ_FFR"){ ?>    
                <?php echo "<script> setTimeout(function(){ alert('Publicacion Agregada!');}, 100);</script>";?>
                <?php header("Refresh:1.5; url=admin");?> <!--Apartado de las leyendas al crear, eliminar o editar publicaciones-->
            <?php  }else if($_REQUEST["info"] == "XCBDWWYURZ22Y"){?>
                <?php echo "<script> setTimeout(function(){ alert('Publicacion Editada!');}, 100);</script>";?>
                <?php header("Refresh:1.5; url=admin");?>
            <?php }else if(($_REQUEST["info"] == "HJFR_$*42JDU385vdE")){?>
                <?php echo "<script> setTimeout(function(){ alert('Publicacion Eliminada!');}, 100);</script>";?>
                <?php header("Refresh:1.5; url=admin");?>
            <?php }else if(($_REQUEST["info"] == "KHNmjdndkX432")){?>
                <?php echo "<script> setTimeout(function(){ alert('Publicaciones Eliminadas!');}, 100);</script>";?>
                <?php header("Refresh:1.5; url=inicio");?>
            <?php }elseif($_REQUEST["info"] == "SSKEEJIOIEM213"){?>
                <?php echo "<script> setTimeout(function(){ alert('Todas las Publicaciones Eliminadas!');}, 100);</script>";?>
                <?php header("Refresh:1.5; url=inicio");?>
            <?php }?>
        <?php }?>
        ...
        <?php if(!empty($master)){ ?>
			<div class="my-btn">
                ...
                <a href="registrar"><div class="btn-fun"><h2 class=dis >Gestionar Administradores</h2></div></a>
                <a href="registrar"><div class="btn-fun"><h2 class=dis >Gestionar Administradores</h2></div></a>
			</div>
		<?php }?>
		<?php if(!empty($master)){ ?>
        ...
        <?php if(!empty($master)){ ?>
            <form action="blog.php" method="POST">
                <button name="destroy" onclick= "return ConfirmAll()" value="<?php echo openssl_encrypt($master["Id"],AES,KEY,0,IV); ?>">Eliminar todas sus publicaciones </button>
            </form>
            <br>
        <?php }?>

        <form action="blog.php" method="POST">
            <button name="Hades" onclick="return hades()" >Eliminar todas las publicaciones</button>
        </form>
        ...
        <section id="service"  class="fondo">
            <div class="container">
                <?php foreach($query as $r){ ?>     <!--Impresion de las publicaciones-->
                    <div class="write largo-publications text-publications">
                        <p>Creada por: <?php echo $r["creador"]; ?></p>
                        ...
                        <img src="img/<?php echo $r["imagen"];?>" width="100px" height="100px">
                        <p>COMENTARIOS</p>
                        <?php
                            $id = $r["Id"]; 
                        ...
                        <?php foreach($comments_added as $j){?>
                            <div>
                                <h5>Fecha de creacion: <?php echo $j["fecha"];?></h5>
